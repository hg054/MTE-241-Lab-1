 ...
 ****************************************************************************/
 ...
 /*
 * Names:
 *   1.  uWaterloo User ID:  h55gu@uwaterloo.ca
 *   2.  uWaterloo User ID:  b372wang@uwaterloo.ca
 */
...
*               to main.c or bst.c.
*****************************************************************
*  1. Passing pointer address instead of pointer.
*  2. Incorrect member reference type (-> to . and vice versa)
*  3. Invalid operands to binary expression
*  4. Passing to incompatible parameter (struct to void)
*  5. Incorrect variable naming.
****************************************************************/
...
void bst_delete ( bsn_t *p_currNode, bsn_t *p_parentNode)
{
	...
	if(p_parentNode->val == p_currNode->val) 
	{
	...
	else if ((p_currNode->right != NULL) & (p_currNode->left != NULL)) 
	{
		...
		p_swapParent->val = p_currNode->val;
	...
	else
	{
		if(p_currNode->left != NULL) //child is to the left
		{
			...
			if(p_parentNode->left->val == p_currNode->val)
			{
